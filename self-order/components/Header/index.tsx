// Header.tsx (updated: show parent title first)
...
interface HeaderProps {
  ...
  activeIndex: number;
  activeParentId?: string | null; // ✅ NEW
  activeCategoryId?: string | null; // keep optional (not required for title anymore)
  collapsedMenu: boolean;
  setCollapsedMenu: React.Dispatch<React.SetStateAction<boolean>>;
  refreshLanguage: () => void;
}

const findCategoryNameById = (categories: IMenuCategory[], id: string | null | undefined) => {
  if (!id) return null;
...
const Header: React.FC<HeaderProps> = ({
  ...
  const { data: userData } = useQuery(ME, {
    onCompleted(data) {
      if (data?.me?.language) {
        ...
        if (countryByLanguage[langCode]) setSelectedCountry(countryByLanguage[langCode]);
      }
    },
  });
  ...
  const availableLanguages = useMemo(() => {
    ...
    if (branchLanguages.length === 0) return countryList;
  ...
  const activeCategoryName = useMemo(() => {
    const parentName = findCategoryNameById(categories, activeParentId);
    if (parentName) return parentName;

    const indexName = categories[activeIndex]?.name;
    if (indexName) return indexName.toLocaleUpperCase();

    const childName = findCategoryNameById(categories, activeCategoryId);
    if (childName) return childName.toLocaleUpperCase();
...
const styles = StyleSheet.create({
  ...
  titleText: { marginLeft: 10, fontSize: 20, fontWeight: '700', textTransform: 'uppercase' },
  rightContainer: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  tableText: { fontSize: 20, color: '#333', fontWeight: '700' },
  cart: { position: 'relative' },
  languageButton: { flexDirection: 'row', padding: 10, alignItems: 'center' },
  flagImage: { height: 28, width: 42, borderRadius: 2 },
  menuItemContainer: { flexDirection: 'row', alignItems: 'center', gap: 6 },
  countryLabel: { fontSize: 14, fontWeight: '700' },
});
